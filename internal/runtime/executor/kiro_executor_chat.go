...
func buildOpenAINonStreamResponse(model, content string, toolCalls []kiroToolCall, detail usage.Detail) []byte {
	finishReason := "stop"
	if len(toolCalls) > 0 {
...
func buildOpenAISSEFinalChunk(id, model, finishReason string, detail usage.Detail) []byte {
